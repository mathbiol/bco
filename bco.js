...
BCO = function(url) {
    ...
    $(document).ready(function() {
        // DOM dependent fun 
        if (document.getElementById('bcoDiv')) {
            // default UI assembly
...
BCO.UI = function(div, bco) {
    // creates UI in target div
    console.log('bcoDiv found, assembling UI')
    var h = '<h2><img id="qrCode" src="qrCode.jpg" hidden=true><br><button id="hideQR" type="button" class="btn btn-primary">QR</button> <a href="http://bit.ly/bcoexec" target="_blank">bit.ly/bcoexec</a></h2><h3 style="color:maroon">Composing Biocompute Objects (BCO) <a href="https://github.com/mathbiol/bco" target="_blank"><i class="fa fa-github-alt" aria-hidden="true"></i><span id="lala" style="font-size:11;vertical-align:middle"> (source)</span></a></h3>'
    h += '<p style="color:green;font-size:small">Alterovitz et. al (2017) Enabling Precision Medicine via standard communication of NGS provenance, analysis, and results. bioRxiv [<a href="https://doi.org/10.1101/191783" target="_blank" style="background-color:yellow">doi.org/10.1101/191783</a>].</p>'
    h += '<hr>'
    h += '<div id="bcoCompDiv"></div>'
    ...
    h += '<hr style="border-color:maroon">'
    h += '<div><button id="showEditedBco" style="color:navy;font-weight:bold;background-color:yellow" class="btn btn-primary">Show edited BCO structure</button> <button id="copyEditedBco" hidden=true style="color:navy;font-weight:bold;background-color:yellow">Copy to Clipboard</button> <button id="saveToGitHub" hidden=true style="color:navy;font-weight:bold;background-color:yellow">Save to GitHub</button>'
    h += '<pre hidden=true></pre></div>'
    h += '<div><button id="downloadEditedBco" style="color:navy;font-weight:bold;background-color:yellow" class="btn btn-primary">Download </button> <span style="color:maroon">with fileName</span> <input style="color:blue"> (only works with <a href="https://www.google.com/chrome/browser/desktop/index.html" target="_blank">Chrome</a> !).</div>'
    ...
    selectParent.selectedIndex = div.selectedIndex || 0
    // recalling previous index if any
    ...
    parentURLinput.onkeyup = function(ev) {
        if (ev.keyCode == 13) {
            // in case user is trying to reload existing URL
            parentURLinput.onchange()
        }
    }
    if (bco.url) {
        // if url was provided already
...
BCO.edit = function(that, p, bco) {
    ...
    el.innerHTML = h
    el.bco = bco
    // passing bco instance as reference available in the edit element
    if (typeof (bc[p]) == "object") {
        // if it's an object we're editing
...
BCO.editParm = function(bco, p, div) {
    ...
    if (typeof (bc[p]) == 'object') {
        ...
        document.getElementById('edit_' + p).onclick = function() {
            BCO.edit(this, p, bco)
        }
    }
...
BCO.bcoEditor = function(bc, bco) {
    if (typeof (bc) == 'string') {
        bcoEditorDiv.innerHTML = '<span style="color:red"> loading ...</span>'
        //+bc
        $.getJSON(bc).then(function(jsn) {
            console.log('editing ' + bc)
            //add automated parameters here ...
            if (!jsn.parentURL) {
                jsn.parentURL = parentURLinput.value
                // ... such as parent provenance
            } else {
                jsn.parentURL += ', ' + parentURLinput.value
            }
        ...
        bco.dt = bc
        // storing teh data in teh instance object
        bcoEditorDiv.innerHTML = ''
        // reseting div content
        Object.getOwnPropertyNames(bc).forEach(function(p) {
            BCO.editParm(bco, p)
            // note bco instance being passed by reference
            //this.bcoJSON=bc
        })
        // create new field
        var newParm = function(bco) {
            ...
            newStruct.onchange = function() {
                newLit.checked = !newStruct.checked
            }
            newLit.onchange = function() {
                newStruct.checked = !newLit.checked
            }
            ...
            $('#newFieldName', newDiv)[0].onkeyup = function(ev) {
                ...
                // allow enter as an equivalent to button click
            }
            //debugger
        }
        newParm(bco)
...
BCO.doneEdit = function(that, cmd) {
    ...
    switch (cmd) {
    case 'done':
        bco.dt[parm] = JSON.parse($('textarea', div)[0].value)
        // update bco.dt parm           
        BCO.editParm(bco, parm, div)
        // reassemble div
